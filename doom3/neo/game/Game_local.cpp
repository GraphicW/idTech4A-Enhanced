...
void idGameLocal::Game_PrecacheMapMedia(void)
{
	common->Printf(
		"=== Map media precache starting ===\n"
	);

	if (mapFile == NULL) {
		common->Warning(
			"Game_PrecacheMapMedia: mapFile is NULL\n"
		);
		return;
	}
	...
	for (int i = 0; i < mapFile->GetNumEntities(); i++) {
		...
		idDict mergedArgs = mapEnt->epairs;

		const char* classname =
			mergedArgs.GetString("classname");
		...
		if (classname != NULL && classname[0] != '\0') {
			...
			if (def) {
				mergedArgs.SetDefaults(&def->dict);
			}
		}

		CacheDictionaryMedia(&mergedArgs);

		entityCount++;
	}

	common->Printf(
		"Precached media for %d map entities\n",
		entityCount
	);

	common->Printf(
		"=== Map media precache finished ===\n"
	);
}

void idGameLocal::Game_PrecacheRuntimeDecls(void)
{
	common->Printf("=== Runtime precache starting ===\n");

	Game_PrecacheMapMedia();

	common->Printf("=== Runtime precache finished ===\n");
}
...
void idGameLocal::InitFromNewMap(const char *mapName, idRenderWorld *renderWorld, idSoundWorld *soundWorld, bool isServer, bool isClient, int randseed)
{
	...
	MapPopulate();
#if 0
	common->Printf("=== PRECACHE ENTRY ===\n");
	Game_PrecacheRuntimeDecls();
	common->Printf("=== PRECACHE EXIT ===\n");
#endif
...
bool idGameLocal::InitFromSaveGame(const char *mapName, idRenderWorld *renderWorld, idSoundWorld *soundWorld, idFile *saveGameFile)
{
	...
	common->Printf(
		"=== SAVEGAME PRECACHE ENTRY ===\n"
	);

	Game_PrecacheRuntimeDecls();

	common->Printf(
		"=== SAVEGAME PRECACHE EXIT ===\n"
	);
	...
	while (kv) {
		if (kv->GetValue().Length()) {
			...
			if (declManager->FindType(DECL_MODELDEF, kv->GetValue(), false) == NULL) {
				...
				// Build .LWO/.ASE collision models during map precache.
				collisionModelManager->LoadModel(kv->GetValue(), false);
			}
		}
...
void idGameLocal::SpawnPlayer(int clientNum)
{
	...
	Printf("ROUTING WARMUP REACHED\n");

	if (clientNum == 0) {

		Printf("ROUTING WARMUP EXECUTING\n");
		...
		for (int i = 0; i < num_entities; i++) {
			...
			if (entity && entity->IsType(idAI::Type)) {

				idAI* ai = static_cast<idAI*>(entity);

				ai->WarmRoutingTo(playerOrigin);

				warmed++;
			}
		}

		Printf("Warmed routing for %d AI\n", warmed);

		Printf("==== AAS Routing Stats After Warmup ====\n");
