...
#define GLSL2_SHADOW_MAPPING_COLOR_MACROS(type) r_usePackColorAsDepth ? type ",_PACK_FLOAT" : type
	...
	ret.Append(GLSL_SHADER_SOURCE("hdr_blur_h",	SHADER_HDR_BLUR_H, &hdrBlurHShader, RETRO_POSTPROCESS_2D_VERT, HDR_BLUR_H_FRAG,	"",	""));
	ret.Append(GLSL_SHADER_SOURCE("hdr_blur_v",	SHADER_HDR_BLUR_V, &hdrBlurVShader,	RETRO_POSTPROCESS_2D_VERT, HDR_BLUR_V_FRAG, "",	"")	);
	ret.Append(GLSL_SHADER_SOURCE("hdr_brightpass", SHADER_HDR_BRIGHTPASS, &hdrBrightPassShader, RETRO_POSTPROCESS_2D_VERT, HDR_BRIGHTPASS_FRAG, "", ""));
	ret.Append(GLSL_SHADER_SOURCE("hdr_luminance", SHADER_HDR_LUMINANCE, &hdrLuminanceShader, RETRO_POSTPROCESS_2D_VERT, HDR_LUMINANCE_FRAG, "", ""));
	ret.Append(GLSL_SHADER_SOURCE("hdr_exposure", SHADER_HDR_EXPOSURE, &hdrExposureShader, RETRO_POSTPROCESS_2D_VERT, HDR_EXPOSURE_FRAG, "", ""));
	ret.Append(GLSL_SHADER_SOURCE("hdr_downsample",	SHADER_HDR_DOWNSAMPLE, &hdrDownsampleShader, RETRO_POSTPROCESS_2D_VERT,	HDR_DOWNSAMPLE_FRAG, "", ""));
...
static void RB_GLSL_GetUniformLocations(shaderProgram_t *shader)
{
	...
	shader->aoIntensity = GL_GetUniformLocation(shader->program, "u_aoIntensity");
	shader->aoRadius = GL_GetUniformLocation(shader->program, "u_aoRadius");
	...
	shader->ssgiBentInfluence =	GL_GetUniformLocation(shader->program, "u_ssgiBentInfluence");
