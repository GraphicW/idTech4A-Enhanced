...
void idRenderWorldLocal::GenerateAllInteractions()
{
	...
	if (r_useInteractionTable.GetBool()) {
		...
		const int entityTableExtra =
			idMath::ClampInt(
				100,
				8192,
				r_interactionTableEntityExtra.GetInteger()
			);

		const int lightTableExtra =
			idMath::ClampInt(
				100,
				2048,
				r_interactionTableLightExtra.GetInteger()
			);

		interactionTableWidth =
			entityDefs.Num() + entityTableExtra;

		interactionTableHeight =
			lightDefs.Num() + lightTableExtra;
		...
		interactionTable =
			(idInteraction**)R_ClearedStaticAlloc(size);
		...
		for (int i = 0; i < this->lightDefs.Num(); i++) {
			...
			for (inter = ldef->firstInteraction;
				inter != NULL;
		...
		common->Printf(
			"interactionTable size: %d bytes\n",
			size);

		common->Printf(
			"interaction table headroom: entities=%d lights=%d\n",
			entityTableExtra,
			lightTableExtra);

		common->Printf(
			"%d interaction take %zd bytes\n",
			count,
