...
#endif
{
	...
	if (cubeFiles != CF_2D) {
		...
		int imageLoadStart = Sys_Milliseconds();
		...
		if (checkForPrecompressed && globalImages->image_usePrecompressedTextures.GetBool()) {
			...
			int ddsStart = Sys_Milliseconds();

			bool loadedDDS = CheckPrecompressedImage(true);

			int ddsElapsed = Sys_Milliseconds() - ddsStart;

			if (ddsElapsed > 10) {
				common->Printf(
					"DDSCHECK %s loaded=%d %d ms\n",
					imgName.c_str(),
					loadedDDS ? 1 : 0,
					ddsElapsed
				);
		...
		int progStart = Sys_Milliseconds();
		...
		int progElapsed = Sys_Milliseconds() - progStart;
		...
		*/
		...
		int genElapsed = Sys_Milliseconds() - genStart;

		if (genElapsed > 10) {
			common->Printf(
				"IMGGENERATE %s %dx%d %d ms\n",
				imgName.c_str(),
				width,
				height,
				genElapsed
