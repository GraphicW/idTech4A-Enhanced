...
idCVar r_hdr("r_hdr", "0", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_BOOL,
	"render the main 3D view into the HDR framebuffer");

static bool hdrResolvePending = false;
...
void RB_ExecuteBackEndCommands(const emptyCommand_t* cmds)
{
	...
	bool classicMirrorSubviewRendered = false;
	...
	for (; cmds; cmds = (const emptyCommand_t*)cmds->next) {
		...
		case RC_DRAW_VIEW:
		{
			const drawSurfsCommand_t* drawCmd =
				(const drawSurfsCommand_t*)cmds;

			const emptyCommand_t* nextCmd =
				(const emptyCommand_t*)cmds->next;

			const bool renderToTexture =
				nextCmd != NULL &&
				nextCmd->commandId == RC_COPY_RENDER;

			const bool fullViewport =
				drawCmd->viewDef->viewport.x1 == 0 &&
				drawCmd->viewDef->viewport.y1 == 0 &&
				drawCmd->viewDef->viewport.x2 == glConfig.vidWidth - 1 &&
				drawCmd->viewDef->viewport.y2 == glConfig.vidHeight - 1;

			const bool classicMirrorSubview =
				drawCmd->viewDef != NULL &&
				drawCmd->viewDef->isMirror &&
				drawCmd->viewDef->isSubview &&
				drawCmd->viewDef->subviewSurface != NULL &&
				drawCmd->viewDef->subviewSurface->material != NULL &&
				drawCmd->viewDef->subviewSurface->material->GetSort() == SS_SUBVIEW;

			const bool mirrorView =
				drawCmd->viewDef != NULL &&
				drawCmd->viewDef->isMirror &&
				drawCmd->viewDef->isSubview &&
				mirrorFramebuffer != NULL &&
				!classicMirrorSubview;

			const bool hdrMainView =
				r_hdr.GetBool() &&
				hdrSceneFramebuffer != NULL &&
				drawCmd->viewDef != NULL &&
				drawCmd->viewDef->viewEntitys != NULL &&
				fullViewport &&
				!renderToTexture &&
				!drawCmd->viewDef->isSubview &&
				!drawCmd->viewDef->isMirror;

			if (classicMirrorSubview && hdrSceneFramebuffer != NULL)
			{
				...
				qglViewport(
					0,
					0,
					hdrSceneFramebuffer->Width(),
					hdrSceneFramebuffer->Height());
			...
			else if (mirrorView)
			{
				...
				qglViewport(
					0,
					0,
					mirrorFramebuffer->Width(),
					mirrorFramebuffer->Height());
			...
			else if (hdrMainView)
			{
				...
				if (classicMirrorSubviewRendered)
				{
					qglClear(
						GL_DEPTH_BUFFER_BIT |
						GL_STENCIL_BUFFER_BIT);
			...
			if (!renderToTexture &&
				...
				!drawCmd->viewDef->isSubview)
			{
				depthStencilRenderer.Blit(
					GL_DEPTH_BUFFER_BIT
				);
			}
#endif

			if (classicMirrorSubview)
			{
				classicMirrorSubviewRendered = true;
			}
			...
			{
				hdrSceneFramebuffer->Unbind();
				classicMirrorSubviewRendered = false;
			}

			if (drawCmd->viewDef->viewEntitys)
			{
				c_draw3d++;
				...
				if (!renderToTexture &&
					...
					!drawCmd->viewDef->isSubview)
				{
					hdrResolvePending = hdrMainView;
					RB_PP_Render();
					hdrResolvePending = false;
				}
#endif
			}
			else
