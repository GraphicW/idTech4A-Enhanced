...
void idRenderThread::HandlePendingImage(void)
{
    ...
    while (GetNextAllocImage(imgData))
    {
        ...
        imgData.image->ActuallyLoadImage(
            imgData.checkForPrecompressed,
            false
        );
        ...
        if (elapsed > 50)
        {
            common->Printf(
                "IMAGELOAD %s %d ms\n",
                imgData.image ? imgData.image->imgName.c_str() : "<null>",
                elapsed
            );
        }
    }

    int queueElapsed = Sys_Milliseconds() - queueStart;

    if (queueElapsed > 10)
    {
        common->Printf(
            "IMAGEQUEUE %d images %d ms\n",
            imageCount,
            queueElapsed
        );
    }
